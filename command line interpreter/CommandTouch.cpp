...
CommandTouch::CommandTouch(std::string name, std::vector <std::string> options, std::vector<std::string> arguments,
...
std::string CommandTouch::executeCommand() {
	touch();
	return "File created";
}

void CommandTouch::printResult(std::string result) {
	return;
}



void CommandTouch::touch() {
	std::string filename;
	filename = getFirstArgument();

    std::ifstream check_file(filename);
    if (check_file.good()) {
        throw new CustomError("Error: file \"" + filename + "\" already exists");
    }
    check_file.close(); 

    std::ofstream new_file(filename);
    if (!new_file) {
        throw new CustomError("Error: file \"" + filename + "\" can't be made"); //za svaki slucaj, npr ime sadrzi neki zabranjeni karakter
    }
